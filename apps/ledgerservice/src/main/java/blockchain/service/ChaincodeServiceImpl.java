...
public class ChaincodeServiceImpl implements ChaincodeService {
	...
	private String chainCodePath = "github.com/luciditee";


	private String chainCodeVersion = "0";
...
//							+ ". Was verified : " + proposalResponse.isVerified());
	...
	public Ledgerentry.BlockchainInfoResponse bcInfo() {
		try {
			checkConfig();
...
//			logger.info("Chainl previous block hash: " + chainPreviousHash);
			...
			return Ledgerentry.BlockchainInfoResponse.newBuilder().setHeight(channelInfo.getHeight())
					...
					.setPreviousBlockHash(chainPreviousHash).build();

		} catch (Exception e) {
			logger.error("ChaincodeServiceImpl | blockchainInfo | "+ e.getMessage());
		}
		return Ledgerentry.BlockchainInfoResponse.newBuilder().build();
	}
