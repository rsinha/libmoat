...
{
  cxxopts::Options options("FX sherlock enclave", "Creates Sherlock's input");

  options
    .show_positional_help()
    .add_options()
      ("help", "Print help")
      ("p,password", "secret value", cxxopts::value<int>())
  ...
  if (!result.count("p") || !result.count("c") || result.count("help")) {
  ...
  luciditee_fx_app::Secret secret;
  secret.set_value((uint64_t) result["p"].as<int>());
