...
uint64_t enclave_test(uint8_t *buf, size_t size)
{
    ...
    LuciditeeFxApp__Secret *secret;
    secret = luciditee_fx_app__secret__unpack(NULL, size, buf);
    assert(secret != NULL);

    _moat_print_debug("parsing proto from enclave...got secret value %" PRIu64 "\n", secret->value);

    luciditee_fx_app__secret__free_unpacked(secret, NULL);
