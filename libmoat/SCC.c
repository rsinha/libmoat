...
void _moat_scc_send(scc_ctx_t *ctx, void *buf, size_t len)
{
    ...
    status = sgx_read_rand(dst_buf + 0, SGX_AESGCM_IV_SIZE);
    assert(status == SGX_SUCCESS);
    status = sgx_rijndael128GCM_encrypt(&(ctx->scc_key),
                                        ...
                                        NULL,
                                        0,
                                        (sgx_aes_gcm_128bit_key_t *) (dst_buf + SGX_AESGCM_IV_SIZE));
    assert(status == SGX_SUCCESS);
...
size_t _moat_scc_recv(scc_ctx_t *ctx, void *buf, size_t len)
{
    ...
    status = sgx_rijndael128GCM_decrypt(&(ctx->scc_key),
                                        ...
                                        NULL,
                                        0,
                                        (sgx_aes_gcm_128bit_key_t *) (ciphertext + SGX_AESGCM_IV_SIZE));
    assert(status == SGX_SUCCESS);
